...
class FragTreeKernel(Kernel):
    ...
    def compute_kernel(self, trees, k_type):
        """
        Parameters:
        -----------
                            ...
                            if id1 in T1.leaves or id2 in T2.leaves:
                                ...
                                for c1 in chl1:
                                    for c2 in chl2:
                                        ...
                                        ee2 = id2+'->'+c2.id
                                        if edict1[ee1] == edict2[ee2]:
                                            s = self._gaussproduct(c1.mass,c2.mass,c1.inten,c2.inten)
...
#                    km[i,j] = sum(T1_v * T2_v)
                    count = 0
                    for t1loss in T1_rloss:
                        if t1loss in T2_rloss:
                            count +=1
                    km[i,j] = count
                    km[j,i] = km[i,j]
...
#                    print "Computing kernel for %d, %d (%s)" % (i,j,km[i,j])
    ...
    def _gaussproduct(self,m1,m2,i1,i2):
        si = 0.00001
        sm = 100000
